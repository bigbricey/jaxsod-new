...
const ROCK_TYPES: RockType[] = [
  {
    name: 'Pea Gravel',
    description: 'Small, rounded stones (3/8") with a natural, earthy look. Easy to walk on and great for drainage.',
    best: 'Best for: Walkways, patios, drainage areas, garden paths',
    tonsPerCubicYard: 1.4,
  },
  {
    name: 'River Rock',
    description: 'Smooth, rounded stones in various sizes (1-3"). Natural colors from creek and river beds.',
    best: 'Best for: Decorative beds, dry creek beds, water features',
    tonsPerCubicYard: 1.4,
  },
  {
    name: 'Crushed Limestone',
    description: 'Angular, compactable stone ideal for bases and structural use. Available in various sizes.',
    best: 'Best for: Driveways, base material, compacted paths',
    tonsPerCubicYard: 1.5,
  },
  {
    name: 'Lava Rock',
    description: 'Lightweight, porous volcanic rock. Excellent drainage and distinctive dark red/black color.',
    best: 'Best for: Decorative beds, xeriscaping, lightweight coverage',
    tonsPerCubicYard: 0.5,
  },
  {
    name: 'Marble Chips',
    description: 'Bright white angular chips that reflect light. Creates a clean, premium aesthetic.',
    best: 'Best for: Decorative borders, premium landscapes, accent areas',
    tonsPerCubicYard: 1.4,
  },
  {
    name: 'Decomposed Granite',
    description: 'Finely crushed granite that compacts to form a firm, natural-looking surface.',
    best: 'Best for: Pathways, patios, rustic landscapes, xeriscape',
    tonsPerCubicYard: 1.5,
  },
]

const DEPTH_OPTIONS = [
  { value: 1, label: '1 inch', note: 'Light cover' },
  { value: 2, label: '2 inches', note: 'Standard' },
  { value: 3, label: '3 inches', note: 'Good coverage' },
  { value: 4, label: '4 inches', note: 'Heavy duty' },
]

const BAG_SIZES = [
  { value: 0, label: "Show bulk only" },
  { value: 0.5, label: '0.5 cu ft bags' },
  { value: 1, label: '1 cu ft bags' },
]
...
function RectangleDiagram({ length, width }: { length: number; width: number }) {
  return (
    <svg viewBox="0 0 260 200" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="rockRectGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#64748b" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#94a3b8" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <rect x="50" y="30" width="160" height="120" rx="4" fill="url(#rockRectGrad)" stroke="#64748b" strokeWidth="2" />
      <line x1="50" y1="168" x2="210" y2="168" stroke="#334155" strokeWidth="1.5" markerEnd="url(#rockArrow)" markerStart="url(#rockArrowR)" />
      ...
      <defs>
        <marker id="rockArrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><path d="M0,0 L8,3 L0,6" fill="#334155" /></marker>
        <marker id="rockArrowR" markerWidth="8" markerHeight="6" refX="0" refY="3" orient="auto"><path d="M8,0 L0,3 L8,6" fill="#334155" /></marker>
      </defs>
    </svg>
  )
}
...
function TriangleDiagram({ base, height }: { base: number; height: number }) {
  return (
    <svg viewBox="0 0 260 200" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="rockTriGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#64748b" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#94a3b8" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <polygon points="130,25 220,165 40,165" fill="url(#rockTriGrad)" stroke="#64748b" strokeWidth="2" strokeLinejoin="round" />
...
function CircleDiagram({ radius }: { radius: number }) {
  return (
    <svg viewBox="0 0 260 220" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="rockCircGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#64748b" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#94a3b8" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <circle cx="130" cy="105" r="80" fill="url(#rockCircGrad)" stroke="#64748b" strokeWidth="2" />
...
export default function RockCalculator() {
  ...
  const roundedCubicYards = Math.ceil(cubicYards * 10) / 10

  // Tons estimate
  const rock = ROCK_TYPES[selectedRock]
  const estimatedTons = Math.ceil(cubicYards * rock.tonsPerCubicYard * 10) / 10
    ...
    <div className="space-y-8 lg:space-y-12">
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-secondary-100 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-primary-700 to-primary-600 px-6 py-5 sm:px-8">
          <h2 className="text-xl sm:text-2xl font-bold text-white">Calculate Your Rock &amp; Gravel Needs</h2>
          <p className="text-primary-100 text-sm mt-1">Select your area shape, depth, and material type below</p>
        </div>
        ...
        <div className="p-6 sm:p-8 space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-secondary-900">Select Your Area Shape</h3>
            </div>
              ...
              <div className="space-y-4">
                {shape === 'rectangle' && (
                  <>
                    ...
                    <NumberInput label="Width" value={dims.width} onChange={(v) => setDim('width', v)} placeholder="e.g. 10" />
                  </>
                )}
                {shape === 'triangle' && (
                  <>
                ...
                {shape === 'irregular' && (
                  <>
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                      ...
                      For irregular areas, measure by dividing into smaller rectangles, calculating each, and adding them together.
                    </div>
                ...
                <div className="flex items-center justify-between bg-secondary-50 rounded-xl p-4 mt-2">
                  <div>
                    ...
                    <p className="text-xs text-secondary-500 mt-0.5">Recommended for edges and uneven areas</p>
                  </div>
                  <button
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-secondary-900">Select Material Depth</h3>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {DEPTH_OPTIONS.map((d) => {
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-secondary-900">Choose Your Material Type</h3>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {ROCK_TYPES.map((r, i) => {
                const active = selectedRock === i
                return (
                  <button
                    key={r.name}
                    onClick={() => setSelectedRock(i)}
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <p className={`font-bold text-base ${active ? 'text-primary-700' : 'text-secondary-900'}`}>{r.name}</p>
                      {active && <FiCheckCircle className="text-primary-600 text-xl flex-shrink-0" />}
                    </div>
                    <p className="text-xs text-secondary-600 mt-2 leading-relaxed">{r.description}</p>
                    <p className="text-xs font-semibold text-primary-700 mt-1.5">{r.best}</p>
                  </button>
                )
              })}
            </div>
        ...
        <div className={`transition-all duration-500 ${hasResult ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
          <div className="bg-gradient-to-br from-primary-700 via-primary-700 to-primary-800 p-6 sm:p-8">
            ...
            <div className="mb-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/10 backdrop-blur rounded-xl p-6 text-center">
                  <p className="text-primary-200 text-xs font-semibold uppercase tracking-wider">Volume Needed</p>
                  <p className="text-3xl font-bold text-white mt-1">{hasResult ? roundedCubicYards.toFixed(1) : '0'}</p>
                  <p className="text-primary-200 text-sm">cubic yards</p>
                </div>
                <div className="bg-white/10 backdrop-blur rounded-xl p-6 text-center">
                  <p className="text-primary-200 text-xs font-semibold uppercase tracking-wider">Est. Weight</p>
                  <p className="text-3xl font-bold text-white mt-1">{hasResult ? estimatedTons.toFixed(1) : '0'}</p>
                  <p className="text-primary-200 text-sm">tons</p>
                </div>
              </div>

              <div className="bg-white/5 backdrop-blur rounded-xl p-3 text-center">
                <p className="text-primary-200 text-xs">
                  <FiInfo className="inline mr-1 -mt-0.5" />
                  Weight estimate based on ~{rock.tonsPerCubicYard} tons/cubic yard for {rock.name}. Actual weight varies by material density and moisture.
                </p>
              </div>

              {/* Optional bag calculator */}
              <div className="bg-white/10 backdrop-blur rounded-xl p-4">
                <p className="text-white text-sm font-semibold mb-2">Buying bagged material? (Optional)</p>
                <p className="text-primary-200 text-xs mb-3">
                  Bag sizes vary by retailer and material. Select your bag size to estimate quantity.
                </p>
            ...
            <p className="text-primary-200 text-xs mb-6">
              Ready for an exact quote? Contact us — we&apos;ll help you choose the right material and arrange delivery.
            </p>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-secondary-100 overflow-hidden">
        <div className="px-6 py-5 sm:px-8 border-b border-secondary-100">
          <h2 className="heading-sm text-secondary-900">Rock &amp; Gravel Types</h2>
          <p className="text-secondary-600 text-sm mt-1">Popular rock and gravel options for Jacksonville, FL landscaping</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-secondary-50">
              <tr>
                <th className="text-left px-6 py-3 font-semibold text-secondary-700">Material</th>
            ...
            <tbody>
              {ROCK_TYPES.map((r) => (
                <tr key={r.name} className="border-b border-secondary-100 hover:bg-primary-50/30 transition-colors">
                  <td className="px-6 py-4 font-semibold text-secondary-900">{r.name}</td>
                  <td className="px-6 py-4 text-secondary-700">{r.description}</td>
                  <td className="px-6 py-4 text-secondary-600 hidden sm:table-cell">{r.best.replace('Best for: ', '')}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="px-6 py-4 sm:px-8 bg-primary-50 border-t border-primary-100">
          <p className="text-sm text-primary-800 font-medium">
            Not sure which material is right for your project? <Link href="/contact" className="underline font-bold hover:text-primary-900">Contact us</Link> or call <a href="tel:9049011457" className="underline font-bold hover:text-primary-900">(904) 901-1457</a> for a free recommendation.
          </p>
        </div>
      </div>
    </div>
