...
type Shape = 'rectangle' | 'triangle' | 'circle' | 'irregular'
...
const MULCH_TYPES: MulchType[] = [
  {
    name: 'Pine Bark',
    description: 'Natural brown bark that decomposes slowly and enriches soil. A popular, budget-friendly choice for Jacksonville gardens.',
    best: 'Best for: General landscaping, garden beds, around trees',
  },
  {
    name: 'Pine Straw',
    description: 'Lightweight and easy to spread. Naturally acidic — great for acid-loving plants like azaleas and camellias.',
    best: 'Best for: Sloped areas, acid-loving plants, budget-friendly coverage',
  },
  {
    name: 'Cypress',
    description: 'Light-colored, aromatic mulch with natural insect-repellent properties. Resists floating in heavy rain.',
    best: 'Best for: Flower beds, walkways, areas prone to heavy rain',
  },
  {
    name: 'Hardwood',
    description: 'Dense, long-lasting mulch that breaks down into rich soil over time. Available in natural and dyed varieties.',
    best: 'Best for: Permanent beds, foundation plantings, mid-range option',
  },
  {
    name: 'Rubber',
    description: 'Made from recycled tires. Extremely long-lasting and does not decompose. Available in multiple colors.',
    best: 'Best for: Playgrounds, permanent installations, premium longevity',
  },
  {
    name: 'Cedar',
    description: 'Aromatic, naturally insect-resistant mulch with a rich reddish-brown color. Premium option that lasts well.',
    best: 'Best for: Premium landscapes, pest-prone areas, ornamental beds',
  },
]

const DEPTH_OPTIONS = [
  { value: 2, label: '2 inches', note: 'Standard refresh' },
  { value: 3, label: '3 inches', note: 'Most common' },
  { value: 4, label: '4 inches', note: 'New beds' },
  { value: 6, label: '6 inches', note: 'Heavy coverage' },
]

const BAG_SIZES = [
  { value: 0, label: "Show cubic yards only" },
  { value: 2, label: '2 cu ft bags' },
  { value: 3, label: '3 cu ft bags' },
]
...
function RectangleDiagram({ length, width }: { length: number; width: number }) {
  return (
    <svg viewBox="0 0 260 200" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="mulchRectGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#d97706" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#f59e0b" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <rect x="50" y="30" width="160" height="120" rx="4" fill="url(#mulchRectGrad)" stroke="#d97706" strokeWidth="2" />
      <line x1="50" y1="168" x2="210" y2="168" stroke="#334155" strokeWidth="1.5" markerEnd="url(#mulchArrow)" markerStart="url(#mulchArrowR)" />
      ...
      <defs>
        <marker id="mulchArrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><path d="M0,0 L8,3 L0,6" fill="#334155" /></marker>
        <marker id="mulchArrowR" markerWidth="8" markerHeight="6" refX="0" refY="3" orient="auto"><path d="M8,0 L0,3 L8,6" fill="#334155" /></marker>
      </defs>
    </svg>
  )
}
...
function TriangleDiagram({ base, height }: { base: number; height: number }) {
  return (
    <svg viewBox="0 0 260 200" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="mulchTriGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#d97706" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#f59e0b" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <polygon points="130,25 220,165 40,165" fill="url(#mulchTriGrad)" stroke="#d97706" strokeWidth="2" strokeLinejoin="round" />
...
function CircleDiagram({ radius }: { radius: number }) {
  return (
    <svg viewBox="0 0 260 220" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="mulchCircGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#d97706" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#f59e0b" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <circle cx="130" cy="105" r="80" fill="url(#mulchCircGrad)" stroke="#d97706" strokeWidth="2" />
...
export default function MulchCalculator() {
  ...
  const [selectedMulch, setSelectedMulch] = useState(0)
  ...
  const handleShapeChange = useCallback((newShape: Shape) => {
    setDims((prev) => {
      const primary = prev.length || prev.base || prev.radius || ''
      const secondary = prev.width || prev.height || ''
  ...
  const totalSqFt = rawSqFt * wasteFactor

  // Convert to cubic yards: (sqft * depth_inches) / 12 = cubic feet, / 27 = cubic yards
  const cubicYards = (totalSqFt * depth) / 12 / 27
  const roundedCubicYards = Math.ceil(cubicYards * 10) / 10 // Round to 1 decimal

  // Bag calculation: 1 cubic yard = 27 cubic feet
  const totalCubicFeet = cubicYards * 27
  const bags = bagSize > 0 ? Math.ceil(totalCubicFeet / bagSize) : 0

  const mulch = MULCH_TYPES[selectedMulch]
  const hasResult = rawSqFt > 0
    ...
    <div className="space-y-8 lg:space-y-12">
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-secondary-100 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-primary-700 to-primary-600 px-6 py-5 sm:px-8">
          <h2 className="text-xl sm:text-2xl font-bold text-white">Calculate Your Mulch Needs</h2>
          <p className="text-primary-100 text-sm mt-1">Select your bed shape, depth, and mulch type below</p>
        </div>
        ...
        <div className="p-6 sm:p-8 space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-secondary-900">Select Your Bed Shape</h3>
            </div>
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-start">
              ...
              <div className="space-y-4">
                {shape === 'rectangle' && (
                  <>
                    <NumberInput label="Length" value={dims.length} onChange={(v) => setDim('length', v)} placeholder="e.g. 20" />
                    <NumberInput label="Width" value={dims.width} onChange={(v) => setDim('width', v)} placeholder="e.g. 6" />
                  </>
                )}
                {shape === 'triangle' && (
                  <>
                    <NumberInput label="Base" value={dims.base} onChange={(v) => setDim('base', v)} placeholder="e.g. 15" />
                    <NumberInput label="Height" value={dims.height} onChange={(v) => setDim('height', v)} placeholder="e.g. 10" />
                  </>
                )}
                {shape === 'circle' && (
                  <NumberInput label="Radius" value={dims.radius} onChange={(v) => setDim('radius', v)} placeholder="e.g. 8" />
                )}
                {shape === 'irregular' && (
                  <>
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                      <FiInfo className="inline mr-1.5 -mt-0.5" />
                      For irregular beds, measure the area by dividing into smaller rectangles, calculating each, and adding them together.
                    </div>
                    <NumberInput label="Total Square Footage" value={dims.totalSqft} onChange={(v) => setDim('totalSqft', v)} placeholder="e.g. 200" unit="sq ft" />
                  </>
                )}
                ...
                <div className="flex items-center justify-between bg-secondary-50 rounded-xl p-4 mt-2">
                  <div>
                    ...
                    <p className="text-xs text-secondary-500 mt-0.5">Recommended for edges and irregular coverage</p>
                  </div>
                  <button
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-secondary-900">Select Mulch Depth</h3>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800 mb-4">
              <FiInfo className="inline mr-1.5 -mt-0.5" />
              <strong>Recommendation:</strong> 2–3 inches for most established beds, 4+ inches for new installations or weed suppression.
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {DEPTH_OPTIONS.map((d) => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 transition-all duration-200 cursor-pointer text-center
                      ${active
                    ...
                    aria-pressed={active}
                  >
                    <p className={`text-lg font-bold ${active ? 'text-primary-700' : 'text-secondary-900'}`}>{d.value}&quot;</p>
                    <p className="text-xs text-secondary-500 mt-0.5">{d.note}</p>
                  </button>
                )
              })}
            </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <span className="w-7 h-7 rounded-full bg-primary-600 text-white text-sm font-bold flex items-center justify-center">4</span>
              <h3 className="text-lg font-bold text-secondary-900">Choose Your Mulch Type</h3>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {MULCH_TYPES.map((m, i) => {
                const active = selectedMulch === i
                return (
                  <button
                    key={m.name}
                    onClick={() => setSelectedMulch(i)}
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <p className={`font-bold text-base ${active ? 'text-primary-700' : 'text-secondary-900'}`}>{m.name}</p>
                      {active && <FiCheckCircle className="text-primary-600 text-xl flex-shrink-0" />}
                    </div>
                    <p className="text-xs text-secondary-600 mt-2 leading-relaxed">{m.description}</p>
                    <p className="text-xs font-semibold text-primary-700 mt-1.5">{m.best}</p>
                  </button>
                )
              })}
            </div>
        ...
        {/* ─ Results Section ──────────────────────────────────── */}
        <div className={`transition-all duration-500 ${hasResult ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
          <div className="bg-gradient-to-br from-primary-700 via-primary-700 to-primary-800 p-6 sm:p-8">
            ...
            <div className="mb-6 space-y-4">
              ...
              <div className="bg-white/10 backdrop-blur rounded-xl p-4 text-center">
                <p className="text-primary-200 text-xs font-semibold uppercase tracking-wider">Total Bed Area</p>
                <p className="text-2xl font-bold text-white mt-1">{Math.ceil(totalSqFt).toLocaleString()}</p>
                <p className="text-primary-200 text-sm">square feet{wasteToggle && rawSqFt > 0 ? ' (incl. 10% waste)' : ''}</p>
              </div>

              {/* Cubic Yards */}
              <div className="bg-white/10 backdrop-blur rounded-xl p-6 text-center">
                <p className="text-primary-200 text-xs font-semibold uppercase tracking-wider">Mulch Needed at {depth}&quot; Deep</p>
                <p className="text-4xl font-bold text-white mt-1">{hasResult ? roundedCubicYards.toFixed(1) : '0'}</p>
                <p className="text-primary-200 text-sm">cubic yards</p>
              </div>

              {/* Optional bag calculator */}
              <div className="bg-white/10 backdrop-blur rounded-xl p-4">
                <p className="text-white text-sm font-semibold mb-2">Buying bagged mulch? (Optional)</p>
                <p className="text-primary-200 text-xs mb-3">
                  Bag sizes vary by brand and retailer. Select your bag size to estimate how many you need.
                </p>
                <div className="flex flex-wrap gap-2">
                  {BAG_SIZES.map((bs) => (
                    <button
                      key={bs.value}
                      onClick={() => setBagSize(bs.value)}
                ...
                {bags > 0 && (
                  <div className="mt-3 text-center">
                    <p className="text-white text-lg font-bold">
                      ≈ {bags} bag{bags !== 1 ? 's' : ''} <span className="text-primary-200 text-sm font-normal">at {bagSize} cu ft each</span>
                    </p>
                  </div>
                )}
              </div>
            ...
            <p className="text-primary-200 text-xs mb-6">
              Ready for an exact quote? Contact us — we&apos;ll help you choose the right mulch and quantity for your project.
            </p>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-secondary-100 overflow-hidden">
        <div className="px-6 py-5 sm:px-8 border-b border-secondary-100">
          <h2 className="heading-sm text-secondary-900">Jacksonville Mulch Types</h2>
          <p className="text-secondary-600 text-sm mt-1">Popular mulch varieties for the Jacksonville, FL area</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-secondary-50">
              <tr>
                <th className="text-left px-6 py-3 font-semibold text-secondary-700">Mulch Type</th>
            ...
            <tbody>
              {MULCH_TYPES.map((m) => (
                <tr key={m.name} className="border-b border-secondary-100 hover:bg-primary-50/30 transition-colors">
                  <td className="px-6 py-4 font-semibold text-secondary-900">{m.name}</td>
                  <td className="px-6 py-4 text-secondary-700">{m.description}</td>
                  <td className="px-6 py-4 text-secondary-600 hidden sm:table-cell">{m.best.replace('Best for: ', '')}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="px-6 py-4 sm:px-8 bg-primary-50 border-t border-primary-100">
          <p className="text-sm text-primary-800 font-medium">
            Not sure which mulch is right for your beds? <Link href="/contact" className="underline font-bold hover:text-primary-900">Contact us</Link> or call <a href="tel:9049011457" className="underline font-bold hover:text-primary-900">(904) 901-1457</a> for a free recommendation.
          </p>
        </div>
      </div>
    </div>
