...
const SOIL_TYPES: SoilType[] = [
  {
    name: 'Topsoil',
    description: 'General-purpose soil for filling, grading, and establishing new lawns. The most common and versatile option.',
    best: 'Best for: Lawn leveling, filling low spots, general landscaping',
  },
  {
    name: 'Garden Mix',
    description: 'Nutrient-rich blend of topsoil, compost, and organic matter. Designed for planting beds and vegetable gardens.',
    best: 'Best for: Raised beds, flower gardens, vegetable gardens',
  },
  {
    name: 'Compost',
    description: 'Fully decomposed organic material that enriches existing soil. Improves drainage, water retention, and nutrient content.',
    best: 'Best for: Soil amendment, top-dressing lawns, enriching garden beds',
  },
  {
    name: 'Fill Dirt',
    description: 'Subsoil material without organic content. Used for structural fill, grading, and raising elevation.',
    best: 'Best for: Grading, filling holes, raising low areas, base material',
  },
  {
    name: 'Sandy Loam',
    description: 'A balanced mix of sand, silt, and clay. Excellent drainage with good nutrient retention — ideal for Florida landscapes.',
    best: 'Best for: New lawn prep, Jacksonville\'s sandy conditions, all-purpose gardening',
  },
]

const DEPTH_OPTIONS = [
  { value: 1, label: '1 inch', note: 'Top-dressing' },
  { value: 2, label: '2 inches', note: 'Light fill' },
  { value: 3, label: '3 inches', note: 'Garden beds' },
  { value: 4, label: '4 inches', note: 'New beds' },
  { value: 6, label: '6 inches', note: 'Lawn prep' },
]
...
function RectangleDiagram({ length, width }: { length: number; width: number }) {
  return (
    <svg viewBox="0 0 260 200" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="soilRectGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#854d0e" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#a16207" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <rect x="50" y="30" width="160" height="120" rx="4" fill="url(#soilRectGrad)" stroke="#854d0e" strokeWidth="2" />
      <line x1="50" y1="168" x2="210" y2="168" stroke="#334155" strokeWidth="1.5" markerEnd="url(#soilArrow)" markerStart="url(#soilArrowR)" />
      ...
      <defs>
        <marker id="soilArrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><path d="M0,0 L8,3 L0,6" fill="#334155" /></marker>
        <marker id="soilArrowR" markerWidth="8" markerHeight="6" refX="0" refY="3" orient="auto"><path d="M8,0 L0,3 L8,6" fill="#334155" /></marker>
      </defs>
    </svg>
  )
}
...
function TriangleDiagram({ base, height }: { base: number; height: number }) {
  return (
    <svg viewBox="0 0 260 200" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="soilTriGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#854d0e" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#a16207" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <polygon points="130,25 220,165 40,165" fill="url(#soilTriGrad)" stroke="#854d0e" strokeWidth="2" strokeLinejoin="round" />
...
function CircleDiagram({ radius }: { radius: number }) {
  return (
    <svg viewBox="0 0 260 220" className="w-full max-w-xs mx-auto">
      <defs>
        <linearGradient id="soilCircGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#854d0e" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#a16207" stopOpacity="0.08" />
        </linearGradient>
      </defs>
      <circle cx="130" cy="105" r="80" fill="url(#soilCircGrad)" stroke="#854d0e" strokeWidth="2" />
...
export default function TopsoilCalculator() {
  ...
  const soil = SOIL_TYPES[selectedSoil]
    ...
    <div className="space-y-8 lg:space-y-12">
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-secondary-100 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-primary-700 to-primary-600 px-6 py-5 sm:px-8">
          <h2 className="text-xl sm:text-2xl font-bold text-white">Calculate Your Topsoil Needs</h2>
          <p className="text-primary-100 text-sm mt-1">Select your area shape, depth, and soil type below</p>
        </div>
        ...
        <div className="p-6 sm:p-8 space-y-8">
              ...
              <div className="space-y-4">
                ...
                {shape === 'irregular' && (
                  <>
                    ...
                    <NumberInput label="Total Square Footage" value={dims.totalSqft} onChange={(v) => setDim('totalSqft', v)} placeholder="e.g. 500" unit="sq ft" />
                  </>
                )}
                ...
                <div className="flex items-center justify-between bg-secondary-50 rounded-xl p-4 mt-2">
                  <div>
                    ...
                    <p className="text-xs text-secondary-500 mt-0.5">Recommended for settling and uneven areas</p>
                  </div>
                  <button
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-secondary-900">Select Soil Depth</h3>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800 mb-4">
              <FiInfo className="inline mr-1.5 -mt-0.5" />
              <strong>Recommendation:</strong> 2–4 inches for garden beds and lawn top-dressing, 4–6 inches for new lawn preparation and raised beds.
            </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-secondary-900">Choose Your Soil Type</h3>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {SOIL_TYPES.map((s, i) => {
                const active = selectedSoil === i
                return (
                  <button
                    key={s.name}
                    onClick={() => setSelectedSoil(i)}
          ...
          <div className="bg-gradient-to-br from-primary-700 via-primary-700 to-primary-800 p-6 sm:p-8">
            ...
            <div className="mb-6 space-y-4">
              ...
              <div className="bg-white/10 backdrop-blur rounded-xl p-6 text-center">
                <p className="text-primary-200 text-xs font-semibold uppercase tracking-wider">{soil.name} Needed at {depth}&quot; Deep</p>
              ...
              {/* Bulk delivery note */}
              <div className="bg-white/5 backdrop-blur rounded-xl p-4 flex items-start gap-3">
                <FiTruck className="text-primary-200 text-xl flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-white text-sm font-semibold">Bulk Delivery</p>
                  <p className="text-primary-200 text-xs mt-1">
                    Topsoil is typically sold in cubic yard increments for bulk delivery. For projects under 1 cubic yard, bagged soil from a garden center may be more convenient. For larger projects, bulk delivery is more economical.
                  </p>
                </div>
              </div>
            </div>
            ...
            <p className="text-primary-200 text-xs mb-6">
              Ready for an exact quote? Contact us — we&apos;ll help you choose the right soil and arrange delivery to your Jacksonville property.
            </p>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-secondary-100 overflow-hidden">
        <div className="px-6 py-5 sm:px-8 border-b border-secondary-100">
          <h2 className="heading-sm text-secondary-900">Topsoil &amp; Soil Types</h2>
          <p className="text-secondary-600 text-sm mt-1">Popular soil options for Jacksonville, FL landscaping projects</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-secondary-50">
              <tr>
                <th className="text-left px-6 py-3 font-semibold text-secondary-700">Soil Type</th>
            ...
            <tbody>
              {SOIL_TYPES.map((s) => (
                <tr key={s.name} className="border-b border-secondary-100 hover:bg-primary-50/30 transition-colors">
                  <td className="px-6 py-4 font-semibold text-secondary-900">{s.name}</td>
